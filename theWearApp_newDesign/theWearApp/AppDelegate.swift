            ...
            if vc.latitude != "" && vc.longitude != ""{
                let geo = vc.latitude  + "%20" + vc.longitude
                                NetworkService.shared.getWeather(currentGEO: geo, completion: {
                                    ...
                                    NotificationService.shared.createnoticreateNotificationAtTime(hour: NotificationService.shared.hourOfPush, minute: NotificationService.shared.minuteOfPush, city : city, text: NotificationService.shared.genereatePush(hour: NotificationService.shared.hourOfPush) ?? "")
                                })} else{
