...
class DayCell: UICollectionViewCell {
    ...
    func configureCell() {
        [substrateView, day, icon, temperature].forEach {addSubview($0)}
        substrateView.frame = CGRect(x: 0, y: 0, width: frame.width, height: frame.height)
        day.frame = CGRect(x: 0, y: 0, width: frame.width, height: 0.4 * frame.height)
        let leftPadding = (frame.width - 0.3 * frame.height) / 2
        icon.frame = CGRect(x: leftPadding, y: 0.4 * frame.height, width: 0.3 * frame.height, height: 0.3 * frame.height)
        temperature.frame = CGRect(x: 0, y: 0.7 * frame.height, width: frame.width, height: 0.3 * frame.height)
    }
